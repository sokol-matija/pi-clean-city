...
describe('LoggingTicketServiceDecorator', () => {
  ...
  describe('updateTicket', () => {
    it('should log before calling wrapped service', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        '[TicketService] Updating ticket: test-123'
      )
      expect(consoleLogSpy).toHaveBeenCalledWith(
    ...
    it('should log success message after successful update', async () => {
      ...
      expect(consoleLogSpy).toHaveBeenCalledWith(
        expect.stringMatching(/successfully in \d+ms/)
      )
    })
    ...
    it('should measure and log execution time', async () => {
      ...
      const successLog = consoleLogSpy.mock.calls.find((call: any[]) =>
        call[0]?.includes('updated successfully')
      )
      expect(successLog).toBeDefined()
      expect(successLog?.[0]).toMatch(/in \d+ms/)
    })
    ...
    it('should log error when wrapped service throws', async () => {
      ...
      await expect(
        decorator.updateTicket(ticketId, changes)
  ...
  describe('getCityServices', () => {
    it('should log before calling wrapped service', async () => {
      ...
      // ASSERT
      expect(consoleLogSpy).toHaveBeenCalledWith(
        '[TicketService] Fetching city services'
      )
    })
    ...
    it('should log success with count and duration', async () => {
      // ARRANGE
      const mockCityServices: Profile[] = [
        { id: '1', username: 'Alice', email: 'alice@city. com', role: 'cityservice', created_at: '2024-01-01', updated_at: '2024-01-01', avatar_url: null },
        { id: '2', username: 'Bob', email:  'bob@city.com', role: 'cityservice', created_at: '2024-01-01', updated_at: '2024-01-01', avatar_url: null }
      ]
  ...
  describe('getStatuses', () => {
    it('should log before calling wrapped service', async () => {
      ...
      // ASSERT
      expect(consoleLogSpy).toHaveBeenCalledWith(
        '[TicketService] Fetching statuses'
      )
    })
  ...
  describe('Decorator does not modify behavior', () => {
    it('should return same value as wrapped service for getCityServices', async () => {
      const mockData: Profile[] = [
        { id: 'w1', username: 'Worker', email: 'w@c.com', role: 'cityservice', created_at: '2024-01-01', updated_at: '2024-01-01', avatar_url: null }
      ]
