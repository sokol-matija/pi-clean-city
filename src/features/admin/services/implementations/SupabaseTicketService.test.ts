...
describe('SupabaseTicketService', () => {
    ...
    describe('getCityServices', () => {
        it('should fetch city sercvices from Supabase with correct filters', async () => {
            //ARRANGE
            mockOrder.mockResolvedValueOnce({ data: mockCityServices, error: null } as any)
        ...
        it('should throw an error when Supabase fails to fetch city services', async () => {
            ...
            mockOrder.mockResolvedValueOnce({ data: null, error: supabaseError } as any)

            //ACT & ASSERT
            await expect(service.getCityServices()).rejects.toThrowError(`Failed to fetch city services: ${supabaseError.message}`)
        })

        it('should return empty array when no city services exist', async () => {
            // ARRANGE
            mockOrder.mockResolvedValueOnce({ data: [], error: null } as any)
        ...
        describe('getStatuses', () => {
            it('should fetch statuses from Supabase ordered by sort_order', async () => {
                //ARRANGE
                mockOrder.mockResolvedValueOnce({ data: mockStatuses, error: null } as any)
            ...
            it('should throw an error when Supabase fails to fetch statuses', async () => {
                ...
                //ACT & ASSERT
                await expect(service.getStatuses()).rejects.toThrowError(`Failed to fetch statuses: ${supabaseError.message}`)
            })
