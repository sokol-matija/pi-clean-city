...
export function useTicketFilters(
    ...
    const filteredReports = useMemo(() => {
        ...
        if (filters.status !== "all") {
            filtered = filtered.filter((r) => r.status_id === parseInt(filters.status))
        }

        if (filters.category !== "all") {
            filtered = filtered.filter((r) => r.category_id === parseInt(filters.category))
        }
