...
import type { ReportDetails } from "@/features/reports/hooks/useReport"
...
function ReportHeader({ report }: Readonly<{ report: ReportDetails }>) {
  return (
    <Card>
      <CardHeader>
        <div className="flex flex-wrap items-start justify-between gap-4">
          <div>
            ...
            <div className="flex flex-wrap items-center gap-2">
              <Badge style={{ backgroundColor: report.status?.color || "#95a5a6", color: "white" }}>
                {report.status?.name}
              </Badge>
...
function PhotosGallery({
  ...
  photos: NonNullable<ReportDetails["photos"]>
...
function CommentsSection({
  ...
  const handleSubmitComment = async (e: React.FormEvent) => {
    ...
    await addComment.mutateAsync({ reportId: String(report.id), content: newComment.trim() })
    setNewComment("")
  }
        ...
        {user ? (
          <form onSubmit={handleSubmitComment} className="space-y-3">
            <Textarea
              ...
              rows={3}
            />
            <Button type="submit" disabled={!newComment.trim() || addComment.isPending} size="sm">
...
function ReportSidebar({ report }: Readonly<{ report: ReportDetails }>) {
  return (
    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-3">
        <div className="space-y-6 lg:col-span-2">
          ...
          {report.photos && (
            <PhotosGallery photos={report.photos} onPhotoClick={setSelectedImage} />
          )}
          <CommentsSection report={report} user={user} addComment={addComment} />
        </div>
