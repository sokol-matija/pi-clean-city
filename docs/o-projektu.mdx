...
Aplikacija koristi Vite kao build alat i React 18.3.1 kao UI biblioteku. Odabrali smo React 18 umjesto React 19 zbog stabilnosti ekosustava i kompatibilnosti s bibliotekama poput react-leaflet koja još nije ažurirana za React 19.

Nismo koristili Next.js jer aplikacija ne zahtijeva server-side rendering (SSR) niti statičku generaciju stranica (SSG). Svi podaci se dohvaćaju na klijentskoj strani putem Supabase klijenta, što čini Next.js API rute nepotrebnima. Jednostavnija arhitektura omogućuje lakše razumijevanje koda za sve članove tima.
...
<AccordionGroup>
  ...
  <Accordion title="Autentifikacija">
    Google OAuth integracija za jednostavnu prijavu. Sesija se automatski osvježava i perzistira između učitavanja stranice.
  </Accordion>
...
<Steps>
  ...
  <Step title="Karta s prijavama">
    Interaktivna karta s obojenim markerima prema statusu
  </Step>
  ...
  <Step title="Komentari">
    Sustav komentara za komunikaciju između građana i službi
  </Step>
...
| Prioritet | Funkcionalnost | Jira reference |
|-----------|---------------|----------------|
| 1 | Tablični prikaz s filterima | SCRM-7 do SCRM-11 |
| 1 | Moje prijave dashboard | SCRM-125 |
| 2 | Dashboard za radnike | SCRM-109 |
| 2 | Ažuriranje statusa | SCRM-52, SCRM-54 |
| 3 | Admin dashboard sa statistikama | SCRM-47, SCRM-48 |
| 4 | Real-time ažuriranja | SCRM-9.x |
| 4 | Notifikacije | SCRM-41, SCRM-42 |

<Note>
  Detaljan pregled implementiranih i planiranih funkcionalnosti dostupan je u `/docs/project/implementation-log.md`.
</Note>
